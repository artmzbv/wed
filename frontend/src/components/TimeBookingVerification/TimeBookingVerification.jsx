...
const TimeBookingVerification = ({
  ...
  const [success, setSuccess] = useState(false); // State to track payment success
  const navigate = useNavigate(); // Initialize useNavigate

  const finalPrice = (() => {
    switch (selectedDuration) {
      case 15:
        return Math.max(0, 30 - discount); // 30 pounds for 15 minutes
      case 30:
        return Math.max(0, 40 - discount); // 40 pounds for 30 minutes
      case 45:
        return Math.max(0, 60 - discount); // 60 pounds for 45 minutes
      case 60:
        return Math.max(0, 75 - discount); // 75 pounds for 60 minutes
      default:
...
// Frontend: handleConfirmPayment function
        ...
        <>
          ...
          <div className='time__confirmation-final'>
            ...
            <div className='time__coupon'>
              ...
              {couponSuccess && <p className="time__success-message">{couponSuccess}</p>} {/* Display coupon success if present */}
            </div>
